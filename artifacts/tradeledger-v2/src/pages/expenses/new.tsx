              ...
              return (
                <button
                  ...
                  className={`p-3 rounded-2xl border-2 flex items-center gap-2.5 text-left transition-all ${
                    isSelected ? "border-primary bg-primary text-white shadow-md" : "border-gray-100 bg-white text-gray-700 hover:border-gray-200"
                  }`}
                >
                  <span className="text-lg leading-none shrink-0">{cat.label.split(" ")[0]}</span>
                  <span className="text-xs font-semibold leading-tight">{cat.label.split(" ").slice(1).join(" ")}</span>
                </button>
              );
            })}
          </div>
